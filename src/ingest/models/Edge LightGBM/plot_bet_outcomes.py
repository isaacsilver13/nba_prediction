"""Generate histograms of bet outcomes by bet side for each model."""
...
INPUT_PATH = "outputs/copilot_bet_logs.csv"
PLOT_DIR = "outputs/bet_outcome_histograms"


def _get_model_suffixes(df: pd.DataFrame) -> List[str]:
    """Extract unique model suffixes from pnl_kelly_* columns."""
...
def _plot_bet_outcomes_by_side(df: pd.DataFrame, suffix: str, output_dir: str) -> str:
    """Create histogram of bet outcomes by bet side (HOME vs AWAY).

    Args:
        df: Bet log dataframe.
        suffix: Model suffix (e.g., 'xgboost_v1').
...
def _plot_combined_outcomes(df: pd.DataFrame, suffix: str, output_dir: str) -> str:
    """Create combined histogram comparing HOME vs AWAY outcomes.
...
def _compute_summary_stats(df: pd.DataFrame, suffix: str) -> dict:
    """Compute summary statistics for a model's bets.
...
def main():
    ...
    if summaries:
        ...
        # Save to CSV
        summary_path = os.path.join("outputs", "bet_outcomes_summary.csv")
